...
class DailyPnLService:
    ...
    async def calculate_daily_pnl(
        ...
        # Calculate aggregates from deals
        net_deposit = 0.0  # Sum of deposits minus withdrawals
        ...
        for deal in deal_history:
            # Check if this is a rebate (REB-tagged)
            if deal.tag == "Rebate":
                rebate += deal.amount
            
            # Categorize deal types
            if deal.action in ('DEPOSIT', 'WITHDRAWAL'):
                net_deposit += deal.amount  # Withdrawals are already negative
            elif deal.action in ('CREDIT', 'CREDIT_OUT', 'CHARGE', 'CORRECTION'):
                net_credit_promotion += deal.amount  # Credits out/charges are already negative
        
        # Step 3: Get total IB commissions and promotion from daily report
        ...
        promotion = target_report.daily_bonus if hasattr(target_report, 'daily_bonus') else 0.0
    ...
    async def calculate_all_logins_pnl(
        ...
        for login, report in reports_by_login.items():
            try:
                ...
                promotion = report.daily_bonus if hasattr(report, 'daily_bonus') else 0.0
